...
class DatasetLoader(data.Dataset):
    ...
    def __init__(self, root='./data/', seed=None, train=True, classes=None, batch_size=128):
        ...
        if train:
            self.depth_input_paths = [root+'ModelNet10_gim/chair/train/'+d for d in os.listdir(root+'ModelNet10_gim/chair/train')]
            # Randomly choose 50k images without replacement
            # self.rgb_paths = np.random.choice(self.rgb_paths, 4000, False)
        else:
            self.depth_input_paths = [root+'ModelNet10_gim/chair/test/'+d for d in os.listdir(root+'ModelNet10_gim/chair/test/')]
